...
registerWhen(register("soundPlay", (pos, name, volume, pitch, categoryName, event) => {
    if (name === "mob.enderdragon.wings") {
        if (!found) {
            new Waypoint("§6Dragon's Lair", pos.x, pos.y, pos.z, 1, 0.84, 0, 0, "world")
...
function checkIfInMineshaft() {
    if (getWorld() === "Mineshaft") {
        timeout = 0;
        setTimeout(function() {
            new Waypoint("§eExit", Math.round(Player.getLastX()), Math.round(Player.getLastY()), Math.round(Player.getLastZ()), 0.01, 0.99, 0.95, 0, "world")
        }, 100);
    }
    else {
        timeout++;
...
registerWhen(register("packetReceived", (packet, event) => { 
    if (isInSkyblock()) {
        if (getZone().replaceAll(" ", "").replaceAll(/[^a-zA-Z]/g, "") == "Carnival") {
            ...
            const postString = blockPos.getX() + "," + blockPos.getY() + "," + blockPos.getZ();
            if (blockState == "minecraft:lit_redstone_lamp") {
                if (blockPos.getX() != -101 && blockPos.getY() != 70 && blockPos.getZ() != 14) {
                    if (lamps[postString]) return;
                    const wp = new Waypoint("", blockPos.getX() +1, blockPos.getY() +1, blockPos.getZ(), 1, 0, 0, 0, "world", true, false, false);
                    lamps[postString] = wp;
                    lampOn = true;
                }
            }
                ...
                if (!lamps[postString]) return;
                lamps[postString].remove();
                delete lamps[postString];
                lampOn = false;
            }
        }
    }
